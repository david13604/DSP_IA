...
class STFTLayer(keras.layers.Layer):
    ...
    def call(self, inputs, fs=44100, cant_muestras=300):
        ...
        stft_result = tf.signal.stft(
            fm_signal, frame_length=128, frame_step=32, fft_length=128
        )
...
class FM_red:
    ...
    def build_model(self):
        ...
        x = keras.layers.MaxPooling2D((9, 9))(x)
        ...
        f_c = keras.layers.Dense(
            self.output_shape // 4, activation="relu", name="f_C"
        )(
            x
        )  # frecuencia carrier
        I = keras.layers.Dense(
            self.output_shape // 4, activation="relu", name="I"
        )(
            x
        )  # indice de modulacion
        A = keras.layers.Dense(
            self.output_shape // 4, activation="relu", name="A"
        )(
            x
        )  # amplitud
        f_m = keras.layers.Dense(
            self.output_shape // 4, activation="relu", name="f_M"
        )(
            x
        )
        ...
        output_layer = STFTLayer(name="stft_layer")([f_c, I, A, f_m])
